...
public class BinaryMapParser extends BinaryParser {
			...
			if (wayCount > 0) {
				...
				if (x < numways) {
					...
					Osmformat.Way w = ways.get((int)(x - 1));
                                        Main.status = Utils.format(nodeCount) + " ways processed... id=" + w.getId();
					System.err.println(Utils.format(wayCount+x) + " ways processed... id=" + w.getId());
				}
			}
			wayCount += numways;
		}
	...
	private void CountNode(long id) {
		...
		if (nodeCount % NODE_STATUS_UPDATE_THRESHOLD == 0) {
                        Main.status = Utils.format(nodeCount) + " nodes processed... id=" + id;
			System.err.println(Utils.format(nodeCount) + " nodes processed... id=" + id);
		}
        ...
        private void countWay(long id)  {
                ...
                if (wayCount % WAY_STATUS_UPDATE_THRESHOLD == 0) {
                        System.err.println(Utils.format(wayCount) + " ways processed... id=" + id);
                        Main.status = Utils.format(wayCount) + " ways processed... id=" + id;
                }
        }
	...
	private void countRelation(long id)  {
		...
		if (relationCount % RELATION_STATUS_UPDATE_THRESHOLD == 0) {
                        Main.status = Utils.format(relationCount) + " relations processed... id=" + id;
                        System.err.println(Utils.format(relationCount) + " relations processed... id=" + id);
		}
	}
