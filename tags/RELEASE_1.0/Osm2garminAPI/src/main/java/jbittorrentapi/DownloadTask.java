...
public class DownloadTask extends Thread implements IncomingListener,
     ...
     * @param s Only set if this client receive a connection request from the
     * remote peer
    ...
    public DownloadTask(Peer peer, byte[] fileID, byte[] myID, boolean init, byte[] bitfield,
        ...
        if (s != null) {
            ...
            //System.out.println("Passive task " + taskid + " created.");
        } else {
            this.peer = peer;
            //System.out.println("Active task " + taskid + " created.");
        }
    ...
    public void run() {
        try {
            ...
            this.fireTaskCompleted("Task " + taskid + " " + this.peer.toString(), this.UNKNOWN_HOST);
        } catch (IOException ioe) {
            this.fireTaskCompleted("Task " + taskid + " " + this.peer.toString(),
    ...
    public synchronized void requestPiece(Piece p) {
        ...
        //System.out.println("Task " + taskid + " requesting piece " + downloadPiece.getIndex());
     ...
     * @return int
     */
    public synchronized long checkDownloaded() {
        long d = new Long(this.downloaded).longValue();
        //this.downloaded = 0;
        return d;
    }
    ...
    public synchronized void connectionClosed() {
        this.clear();
        this.fireTaskCompleted("Task " + taskid + " " + this.peer.toString(), this.CONNECTION_REFUSED);
    }
     ...
     * message from the remote peer since more that 3 minutes, the remote peer
     * is
     * considered as dead, and we close the connection, then inform the
     * DownloadManager that this connection timed out...
     *
     * Otherwise, inform the DownloadManager that this task is still alive and
     * has
    ...
    public synchronized void keepAliveSent() {
        if (System.currentTimeMillis() - this.lmrt > 180000) {
            this.clear();
            this.fireTaskCompleted("Task " + taskid + " " + this.peer.toString(), this.TIMEOUT);
            return;
        }
        ...
        if (m.getType() == PeerProtocol.HANDSHAKE) {
            ...
            switch (message.getType()) {
                ...
                case PeerProtocol.UNCHOKE:
                    /*
                     * Change the choking state to false, meaning this client
                     * now
                     * accepts request messages from this client.
                     * If this task was already downloading a piece, then
                     * continue.
                     * Otherwise, advertise DownloadManager that it is ready to
                     * do so
                     */
                ...
                case PeerProtocol.INTERESTED:
                     ...
                     * meaning this peer will start downloading from this client
                     * if
                     * it is unchoked
                     */
                ...
                case PeerProtocol.NOT_INTERESTED:
                    /*
                     * Change the interested state of the remote peer to true,
                     * meaning this peer will not start downloading from this
                     * client
                     * if it is unchoked
                     */
                ...
                case PeerProtocol.HAVE:
                    /*
                     * Update the peer piece list with the piece described in
                     * this
                ...
                case PeerProtocol.REQUEST:
                    /*
                     * If the peer is not choked, advertise the DownloadManager
                     * of
                     * this request. Otherwise, end connection since the peer
                     * does
                    ...
                    if (!this.peer.isChoked()) {
                        ...
                        this.firePeerRequest(this.peer.toString(),
                                ...
                                Utils.byteArrayToInt(
                                Utils.subArray(message.getPayload(), 8, 4)));
                    } else {
                        this.fireTaskCompleted("Task " + taskid + " " + this.peer.toString(), this.MALFORMED_MESSAGE);
                    }
                    break;

                case PeerProtocol.PIECE:
                    /**
                     * Sets the block of data downloaded in the piece block list
                     * and
                     * update the peer download rate. Removes the piece block
                     * from
                    ...
                    this.changeState(this.DOWNLOADING);
                    //System.out.println("Task " + taskid + " received piece " + downloadPiece.getIndex()
                    //        + " from " + begin + " " + (data.length - 8) + "bytes");
                    break;
     ...
     * messages
     * This is here that are taken the most important decisions about the
     * messages to
    ...
    private synchronized void changeState(int newState) {
        ...
        switch (newState) {
            ...
            case WAIT_BLOCK:
                 ...
                 * performance.
                 * If only sending 1 request an waiting, it is a loss of time
                 * and
            ...
            case READY_2_DL:
                /**
                 * Advertise the DownloadManager that this task is ready to
                 * download
                 */
                this.firePeerReady(this.peer.toString());
                break;
            case DOWNLOADING:
                 ...
                 * request size
                 * is 0, then we have downloaded all the piece blocks and we can
                 * verify
    ...
    public synchronized void end() {
        ...
        //System..println("Task " + taskid + " stopped.");
    }
