...
public class ConfigurationLogic extends ProductConfigurationLogic {
    ...
    public void install(Progress progress) throws InstallationException {
        ...
        if (SystemUtils.isCurrentJava64Bit()) {
            replaceString += " -J-XX:PermSize=64M -J-XX:MaxPermSize=256m"; // NOI18N
        }
        
        File etcDir = new File(installLocation, "etc");  // NOI18N
        File startupConfigFile = new File(etcDir, 
                ResourceUtils.getString(ConfigurationLogic.class, "CL.app.name") + ".conf");  // NOI18N
 
        LogManager.log("Updating startup -J-Xmxnnnnm parameter in " + startupConfigFile + " with " + replaceString);  // NOI18N

        filterConfigFile(startupConfigFile, "-J-Xmx[0-9]+m", replaceString);  // NOI18N
        
        String javaHome = SystemUtils.getCurrentJavaHome().getAbsolutePath();
        filterConfigFile(startupConfigFile, "#jdkhome=\"/path/to/jdk\"", 
                "jdkhome=\"" + javaHome.replace("\\", "/") + "\"");  // NOI18N
    ...
    private void filterConfigFile(File startupConfigFile, String regex, String replaceString) throws InstallationException {
        try {
            int length = (int)startupConfigFile.length();
            byte[] b = new byte[length];
            InputStream is = new BufferedInputStream(new FileInputStream(startupConfigFile));
            int l = 0, ll = 0;
            while (l >= 0) {
                l = is.read(b, ll, length - ll);
                ll += l;
                if (l==0) {
                    l = -1;
                }
            ...
            s = s.replaceAll(regex, replaceString);
            if (! SystemUtils.isWindows()) {
                s = s.replace("\r\n", "\n");
            }
            b = s.getBytes();
            OutputStream os = new BufferedOutputStream(new FileOutputStream(startupConfigFile));
            os.write(b);
            os.flush();
            os.close();
        } catch (IOException ex) {
            throw new InstallationException(ex.getLocalizedMessage());
        }
    }
}
