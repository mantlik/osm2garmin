...
public class Peer {
    ...
    public float getDLRate(boolean reset) {
        float sec = (float) Math.max((System.currentTimeMillis() - lastDL) / 1000, 0.1);
        if (reset) {
            float tmp = this.dlrate;
            resetDL();
            return tmp / sec;
        } else {
            return this.dlrate / sec;
        }
    ...
    public float getULRate(boolean reset) {
        float sec = (float) Math.max((System.currentTimeMillis() - lastUL) / 1000,0.1);
        if (reset) {
            float tmp = this.ulrate;
            resetUL();
            return tmp / sec;
        } else {
            return this.ulrate / sec;
        }
    }
